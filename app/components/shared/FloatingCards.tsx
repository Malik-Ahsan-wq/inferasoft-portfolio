    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      >
        <motion.div
          ...
          className="rounded-2xl bg-white/95 backdrop-blur-md px-5 py-4 shadow-xl border border-white/20"
        >
      ...
      {/* Right-to-left floating card strip */}
      <div className="absolute bottom-[6%] left-0 right-0 hidden sm:block">
        ...
        >
          {[...cards, ...cards].map((card, index) => (
            <div
              key={`${card.id}-${index}`}
              className="flex-shrink-0 rounded-2xl bg-white/90 backdrop-blur-md px-5 py-3.5 shadow-lg border border-white/20 min-w-[200px]"
            >
              {card.avatars ? (
                <>
